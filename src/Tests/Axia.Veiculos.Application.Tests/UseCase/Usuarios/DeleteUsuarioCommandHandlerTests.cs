...
public class DeleteUsuarioCommandHandlerTests
{
    ...
    public DeleteUsuarioCommandHandlerTests()
    {
        ...
        _handler = new DeleteUsuarioCommandHandler(_usuarioRepositoryMock.Object);
    }

    [Fact]
    public async Task Handle_UsuarioExistente_DeveRemover()
    {
        //Arrange
        var id = Guid.NewGuid();
        var usuario = Usuario.Create("Teste", "teste", "hash", Role.User);
